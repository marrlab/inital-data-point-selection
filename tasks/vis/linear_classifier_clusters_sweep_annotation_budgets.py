...
from src.vis.helpers import format_criterium, format_dataset
from src.models.build_data_path import get_vis_folder_path
from src.vis.constants import DATASETS_ORDER, OUR_CRITERIA_ORDER_NO_RANDOM, ANNOTATION_BUDGETS 

FONT_SIZE = 32

@hydra.main(version_base=None, config_path='../../conf', config_name='linear_classifier_clusters_sweep_annotation_budgets')
def main(cfg: DictConfig):
    ...
    if cfg.checkpoint_dir is None:
        ...
        def transform_runs_general(df):
            ...
            df['train_samples'] = df.apply(
                lambda r: r['config']['training']['train_samples'], axis=1)
    ...
    df_general = df_general.melt(
        id_vars=['dataset', 'ssl', 'criterium', 'clusters', 'train_samples'],
    ...
    row_order = DATASETS_ORDER 
    col_order = ANNOTATION_BUDGETS
    hue_order = OUR_CRITERIA_ORDER_NO_RANDOM
    # for ssl in df_general['ssl'].unique():
    for ssl in df_general['ssl'].unique():
        df_ssl = df_general[
            (df_general['ssl'] == ssl) &
            (df_general['metric_name'] == metric_name)
        ]
        ...
        handles_criteria, labels_criteria = sns.relplot(
            ...
            palette='colorblind',
            y='metric_value',
            row='dataset', row_order=row_order,
            col='train_samples', col_order=col_order,
            kind='line',
            errorbar='sd',
        ...
        for i, row in enumerate(fig.axes):
            ...
            for j, ax in enumerate(row):
                ...
                y_min = np.floor(10 * df_ssl[df_ssl['dataset'] == dataset]['metric_value'].min()) / 10
                y_max = np.ceil(10 * (ceiling_mean + ceiling_std)) / 10
                y_step = 0.1
                ...
                df_random = df_ssl[
                    ...
                    (df_ssl['train_samples'] == train_samples)
                ]['metric_value']
                ...
                if j == (len(col_order) // 2):
                    title += format_dataset(dataset)
                title += f'\n\nbudget: {train_samples}'
                ax.set_title(title, pad=20)

                # hacky way to have nice xticks
                x_step = int(train_samples / 4) 
                ax.set_xticks(np.arange(0, train_samples + x_step, x_step, dtype=np.int32))

                ax.set_xlabel('k-means clusters', fontsize=FONT_SIZE)

                ax.set_xlim((0, train_samples))

                if cfg.show_legend and i == len(row_order) - 1 and j == len(col_order) - 1:
                    # creating the legend
                    ...
                    ax.legend(
                        ...
                        ncols=len(handles),
                        loc='lower right', bbox_to_anchor=(0.82,0),
                        columnspacing=0.8,
                        framealpha=1.0, 
        ...
        fig.savefig(f"all_{ssl.replace('/', '_')}.pdf")
        plt.clf()
