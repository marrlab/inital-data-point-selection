...
def main(cfg: DictConfig):
    ...
    df_mean['dataset'] = pd.Categorical(df_mean['dataset'], DATASETS_ORDER)
    df_mean = df_mean.sort_values('dataset')
    print(df_mean)

    # What will be the angle of each axis in the plot? (we divide the plot / number of variable)
    angles = [
        n / float(len(DATASETS_ORDER)) * 2 * np.pi 
        for n in range(len(DATASETS_ORDER))
    ]
    angles += angles[:1]

    for ssl in ['simclr/v1']:
        fig, axes = plt.subplots(1, len(REQUIRED_METRICS_SHORT_NAME), subplot_kw=dict(projection='polar'), figsize=(15,3))
        handles = []
        labels = []
        for i, required_metric_short_name in enumerate(REQUIRED_METRICS_SHORT_NAME):
            df_mean_cur = df_mean[
                (df_mean['ssl'] == ssl)
            ][['dataset', 'criterium', required_metric_short_name]]

            ax = axes[i]
            ax.set_title(required_metric_short_name, pad=15)
            ...
            ax.set_xticks(angles[:-1], DATASETS_ORDER)

            # Draw ylabels
            y_min = df_mean_cur[required_metric_short_name].min().min()
            y_min = np.floor(10 * y_min) / 10
            y_max = df_mean_cur[required_metric_short_name].max().max()
            ...
            # random 
            values = df_mean_cur[df_mean_cur['criterium'] == 'random'][required_metric_short_name].values.flatten().tolist()
            values += values[:1]
            ...
            for criterium in CRITERIA_ORDER_NO_RANDOM:
                values = df_mean_cur[df_mean_cur['criterium'] == criterium][required_metric_short_name].values.flatten().tolist()
                values += values[:1]
            ...
            # ceiling
            values = df_mean_cur[df_mean_cur['criterium'] == 'full_data'][required_metric_short_name].values.flatten().tolist()
            values += values[:1]
